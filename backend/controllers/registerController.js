...
exports.registerUser = async (req, res) => {
  ...
  try {
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: Arial, sans-serif; border:1px solid #ddd; padding:20px;">
            ...
            <table cellpadding="8" cellspacing="0" border="1" style="border-collapse: collapse; margin-top:10px;">
              <thead style="background:#f2f2f2;">
                <tr><th>Name</th><th>Email</th><th>Phone</th><th>Remarks</th></tr>
              </thead>
              <tbody>
                <tr>
                  <td>${name}</td>
            ...
            <p style="margin-top:20px; font-size:13px; color:#555;">This is an automated registration alert for Edzest Events.</p>
          </div>`
      });
    } catch (e) {
    ...
    try {
          ...
          <div style="font-family: Arial, sans-serif; border:1px solid #ddd; padding:20px;">
            ...
            <table cellpadding="8" cellspacing="0" border="1" style="border-collapse: collapse; margin-top:10px; width:100%;">
              <tbody>
                ...
                <tr><td><strong>Speaker</strong></td><td>${event.speaker}</td></tr>
                <tr><td><strong>Join Link</strong></td><td><a href="${event.link}" target="_blank">${event.link}</a></td></tr>
              </tbody>
            </table>
            <p style="margin-top:20px;">We're excited to have you join us.</p>
            <p style="font-size:13px; color:#888;">
      ...
      console.error("❌ Failed to send user email:", e.message);
    }
...
exports.downloadRegistrations = async (req, res) => {
  try {
    const registrations = await Registration.find({ eventId: req.params.eventId });
    const data = registrations.map(r => ({
      Name: r.name, Email: r.email, Phone: r.phone, Remarks: r.remarks
    }));
    const xlsx = require('xlsx');
    const wb = xlsx.utils.book_new();
    const ws = xlsx.utils.json_to_sheet(data);
