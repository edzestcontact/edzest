...
//   const [currentCardIndex, setCurrentCardIndex] = useState(() => {
//     return parseInt(localStorage.getItem(`currentCardIndex_${selectedCategory}_${selectedType}`)) || 0;
//   });
//   const [answeredCards, setAnsweredCards] = useState(() => {
//     return JSON.parse(localStorage.getItem(`answeredCards_${selectedCategory}_${selectedType}`)) || {};
//   });
//   const [score, setScore] = useState(() => {
//     return parseInt(localStorage.getItem(`score_${selectedCategory}_${selectedType}`)) || 0;
//   });
//   const [progress, setProgress] = useState(() => {
//     return parseFloat(localStorage.getItem(`progress_${selectedCategory}_${selectedType}`)) || 0;
//   });  
...
//       localStorage.setItem(`currentCardIndex_${selectedCategory}_${selectedType}`, currentCardIndex);
//       localStorage.setItem(`answeredCards_${selectedCategory}_${selectedType}`, JSON.stringify(answeredCards));
//       localStorage.setItem(`score_${selectedCategory}_${selectedType}`, score);
//       localStorage.setItem(`progress_${selectedCategory}_${selectedType}`, progress);
//     }
//   }, [selectedCategory, selectedType, currentCardIndex, answeredCards, score, progress]);
  

//   const handleCategoryChange = (category, type) => {
//     console.log(`Switching to Category: ${category}, Type: ${type}`); // Debugging
  
//     if (selectedCategory && selectedType) {
//       // ✅ Store previous category data correctly
...
//       localStorage.setItem(`score_${selectedCategory}_${selectedType}`, score);
//       localStorage.setItem(
//         `progress_${selectedCategory}_${selectedType}`,
//         progress
//       );
//     }
  
//     // ✅ Retrieve saved data for the new category
//     const savedCurrentCardIndex = parseInt(
//       localStorage.getItem(`currentCardIndex_${category}_${type}`)
//     ) || 0;
//     const savedAnsweredCards = JSON.parse(
//       localStorage.getItem(`answeredCards_${category}_${type}`)
//     ) || {};
//     const savedScore =
//       parseInt(localStorage.getItem(`score_${category}_${type}`)) || 0;
//     const savedProgress =
//       parseFloat(localStorage.getItem(`progress_${category}_${type}`)) || 0;
...
//     return (
//       <div key={index} className="d-flex align-items-center flex-shrink-0">
//         <Nav.Item>
//           <Nav.Link
//             active={isActive}
//             onClick={() => handleTabClick(tabPath)}
//             className={`px-3 py-1 ${isActive ? "border border-primary bg-light fw-bold rounded" : "border-0"}`}
//             style={{
//               cursor: "pointer",
//               color: "#4748ac",
//             }}
...
// <ProcessCategories
//       categories={processCategories}
//       onSelectCategory={(category) => handleCategoryChange(category, "Process")}
//     />
//           </>
//         )}
...
//         <AgileCategories
//   categories={agileCategories}
//   onSelectCategory={(category) => handleCategoryChange(category, "Agile")}
// />
//           </>
//         )}
...
//       onSelectCategory={(category) => handleCategoryChange(category, "Project Management Foundation")}
//     />
//             </>
//           )}
...
//       onSelectCategory={(category) => handleCategoryChange(category, "Knowledge Area")}
//     />
//           </>
//         )}
...
//       onSelectCategory={(category) => handleCategoryChange(category, "Performance Domain")}
//     />
//           </>
//         )}
...
const FlashMain = () => {
  ...
  const handleCategoryChange = (category, type) => {
    ...
    if (selectedCategory && selectedType) {
      ...
      localStorage.setItem(
        `currentCardIndex_${normalizeKey(selectedCategory)}_${normalizeKey(selectedType)}`,
        currentCardIndex
      );
      localStorage.setItem(
        `answeredCards_${normalizeKey(selectedCategory)}_${normalizeKey(selectedType)}`,
        JSON.stringify(answeredCards)
      );
      localStorage.setItem(
      ...
      localStorage.setItem(
        `progress_${normalizeKey(selectedCategory)}_${normalizeKey(selectedType)}`,
        progress
      );
    }
  
    // ✅ Debugging: Check the key format before retrieving data
    console.log("Retrieving Data for:", `currentCardIndex_${normalizeKey(category)}_${normalizeKey(type)}`);
    
    const savedCurrentCardIndex = parseInt(
      localStorage.getItem(`currentCardIndex_${normalizeKey(category)}_${normalizeKey(type)}`)
    ) || 0;
  
    const savedAnsweredCards = JSON.parse(
      localStorage.getItem(`answeredCards_${normalizeKey(category)}_${normalizeKey(type)}`) || "{}"
    );
  
    const savedScore = parseInt(
      localStorage.getItem(`score_${normalizeKey(category)}_${normalizeKey(type)}`)
    ) || 0;
  
    const savedProgress = parseFloat(
      localStorage.getItem(`progress_${normalizeKey(category)}_${normalizeKey(type)}`)
    ) || 0;
...
useEffect(() => {
  if (selectedCategory && selectedType) {
    ...
    localStorage.setItem(`currentCardIndex_${categoryKey}_${typeKey}`, currentCardIndex);
    localStorage.setItem(`answeredCards_${categoryKey}_${typeKey}`, JSON.stringify(answeredCards));
